...
# so SecurityHeadersMiddleware (added last) wraps everything and always
# stamps headers, even on responses produced by the CSRF/session layers.
...
async def dashboard(request: Request):
    ...
    with get_db() as con:
        ...
        # ── Secrets are never sent to the browser for non-privileged users. ────
        # Only users with explicit manage_* permission (or admins) get the raw
        # token back so it can be edited in the Settings form; everyone else
        # gets a masked placeholder so the value never appears in page source.
        can_manage_cf = user["role"] == "admin" or "manage_cloudflare" in user_perms
        can_manage_crafty = user["role"] == "admin" or "manage_settings" in user_perms
